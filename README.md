...
5. 利用sort包的slice函数, 对3个实例, 按照面积特征进行就地排序, 然后遍历打印
   ```go
...
6. 以上排序的问题是每次排序都要调用方法重新做计算. 解决方法是把计算得到的面积放在自己实例的属性中,以后直接比较属性值做排序
...
### 4.3 为fib递归版提供一个cache
1. 先把fib递归版写出来
2. 定义一个map,类型map[int]int, key是项目,value是对应的值,有初值fib(0)=0, fib(1)=1, fib(2)=2
3. switch先判断
4. 对map进行定位,如果map[n]有值就直接返回,如果map[n]没有值,就递归计算存到map[n]里面,在返回.
5. 可以把v抽出来, 可以把return抽出来
6. 打印验证是否存储
### 4.4 时间打印8年前今天上午9点30分
1. 取当前时间，减8年
2. 时间格式化为: 2006/01/02
3. 时区格式化为：-0700
4. Sprintf拼接成一个字符串，打印
5. 把字符串解析成时间实例。
6. 拿到时间实例进行计算
   * 算毫秒时间戳
   * format成字符串
   * 打印那一天是星期几
   * 打印这一天是一年中的第几周
   * 计算距离今天有多少天，计算小时除以24，向上取整
### 5.1 数字组合
有4个数字1,2,3,4, 打印可以组成多少个不重复的3位数. 要求3位数不同位不能有重复的数字. 112, 212不可以
1. 需要i,j,k三层循环: i=1, j=1, k=1~4; i=1, j=2, k=1~4
2. 方法一：写条件判断，如果i!=j, i!=k, j!=k 则打印 ijk
3. 使用bitmap来解决
   拿走第一个元素，对应的bitmap=1，不用了bitmap还原到0
   例如：
   第一位拿4 对应的bitmap=1
     第二位拿1 对应的bitmap=1
       第三位拿2 对应的bitmap=1
     ...
     第二位拿3 对应的bitmap=1
       ...
       第三位拿2 对应的bitmap=1
         ...
         3不用了，bitmap=0
     第二位不用3了 对应的bitmap=0
     第二位拿不到4 因为4的bitmap=1
     第二位退出循环
     第一位4不用了，bitmap=0
     第一位在开始循环拿1
### 5.2 字母组合
使用英文小写字母随机生成一个大于3个字符的字符串, 在把这个字符串逆序打印
1. 写一个26个字母的字符串
2. 函数要求输入字母的长度, 循环里面使用随机数取字符串的第几个字母, 用building包来拼接.
3. 主要是字符串逆序打印的方案
   (1) 拿到的字符串对称位置交换
      ```bash
      a       1      0
      ...
      abcdef  6      3
      abcdefg 7      3
      # 规律: 交换的次数: a. length=双数, length/2  
      #                  b. length=奇数, (length-1)/2
      ```
   (2) 传进来的string, 要转换成[]byte字节流, 这样才能通过索引交换
   (3) swap的次数如上
   (4) for循环进行swap
   (5) 最后在转换成string
### 5.3 合并字符串
   给出两个英文小写字符串, 依次字符交叉合并成新字符串, 多出的部分追加在新字符串的末尾
   ```bash
   a[0]b[1]c[2], x[0]y[1]z[2] -> a[0]x[1]b[2]y[3]c[4]z[5]
   长度一样: word1[0]->new[0] word1[1]->new[2] word1[2]->new[4] 
            2倍自己的索引
            word2[0]->new[1]  word2[1]->new[3] word2[2]->new[5]
            2倍自己的索引+1

   a[0]b[1], x[0]y[1]z[2]  -> a[0]x[1]b[2]y[3]z[4]
            word1[0]->new[0] word1[1]->new[2]
            2倍自己的索引
            word2[0]->new[1] word2[1]->new[3] word2[2]->new[]
            2倍自己的索引+1
            
   a[0]b[1]c[2]d[3], x[0]y[1] -> a[0]x[1]b[2]y[3]c[4]d[5]
            word1[0]->new[0] word1[1]->new[2] ...
            2倍自己的索引
            word2[0]->new[1] word2[1]->new[3] ...
            2倍自己的索引+1

   ```
   * 思路
   (1) 插入的次数=最小字符串的长度
   (2) 根据规律: new字符串的[2i]=word1的[i]
                new字符串的[2i+1]=word2的[i]
   (3) 构成一个互相插入的字符串
   (4) 最长的字符串, 截取后面超出的部分, 和之前的字符串做拼接
